...
def load_lock(root: Path = ROOT) -> dict:
    ...
    if value.get("schema_version") != 1 or value.get("system") != "x86_64-linux":
...
def clone_component(root: Path = ROOT) -> None:
    ...
    subprocess.run(["git", "clone", "--no-checkout", "https://github.com/" + entry["repository"] + ".git", str(path)], check=True)
